...
import { searchTires, toSlug, getBrandBySlug } from "@/lib/db";
...
export async function generateMetadata({
  ...
  params: Promise<{ make: string; model: string; year: string; brand: string }>;
}): Promise<Metadata> {
  const { make, model, year, brand: brandSlug } = await params;
  ...
  return {
    title: `${brandRow.make_name} Tires for ${year} ${makeName} ${modelName} — Ship Free | Ship.Tires`,
    description: `Shop ${brandRow.make_name} tires for your ${year} ${makeName} ${modelName}. Compatible sizes: ${sizeText}. Free shipping on every order at Ship.Tires.`,
    alternates: { canonical: `https://ship.tires/tires/vehicle/${make}/${model}/${year}/brand/${brandSlug}` },
  };
}
...
export default async function VehicleYearBrandPage({
  params,
}: {
  params: Promise<{ make: string; model: string; year: string; brand: string }>;
}) {
  ...
  const breadcrumb = buildBreadcrumbSchema([
    ...
    { name: brandName, url: `https://ship.tires/tires/vehicle/${make}/${model}/${year}/brand/${brandSlug}` },
  ]);

  const vehicleSchema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `${brandName} Tires for ${year} ${makeName} ${modelName}`,
    description: `Shop ${brandName} tires for your ${year} ${makeName} ${modelName}. ${tireGroups.length} models across ${compatibleSizes.length} sizes. Free shipping.`,
    url: `https://ship.tires/tires/vehicle/${make}/${model}/${year}/brand/${brandSlug}`,
    about: {
      "@type": "Vehicle",
        ...
        <div className="bg-navy py-12 text-white">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-2 text-sm text-gray-400">
              ...
              <span>/</span>
              <Link href={`/tires/vehicle/${make}/${model}/${year}`} className="hover:text-white transition-colors">{year}</Link>
              <span>/</span>
              <span className="text-gray-300">{brandName}</span>
            </div>
            <h1 className="mt-4 text-3xl font-bold sm:text-4xl lg:text-5xl">
              {brandName} Tires for {year} {makeName} {modelName}
            </h1>
        ...
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
            <p className="text-gray-600 leading-relaxed">
              Browse {brandName} tires compatible with your {year} {makeName} {modelName}. Available sizes include{" "}
          ...
          {[...bySeason.entries()].map(([season, tires]) => (
            <div key={season} className="mb-12">
              <h2 className="text-2xl font-bold text-gray-900 border-b border-gray-200 pb-3 mb-6">
                {brandName} {season} Tires for {year} {makeName} {modelName} ({tires.length})
              </h2>
          ...
          <div className="mt-8 rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {brandName} Tires for Other {makeName} {modelName} Years
            </h2>
            <div className="flex flex-wrap gap-2">
              {otherYears.map((y) => (
                <Link
                  key={y}
                  href={`/tires/vehicle/${make}/${model}/${y}/brand/${brandSlug}`}
                  className="rounded-full border border-gray-200 bg-gray-50 px-4 py-1.5 text-sm font-medium text-gray-700 hover:border-safety-orange hover:text-safety-orange transition-colors"
                >
          ...
          <div className="mt-8 rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {compatibleSizes.map((s) => {
                const sizeSlug = s.toLowerCase().replace(/\//g, "-");
                return (
                  <Link
                    key={s}
          ...
          <div className="mt-8 rounded-xl bg-navy p-8 text-center text-white">
            <h3 className="text-xl font-bold">Not sure which {brandName} tire is right for your {year} {makeName} {modelName}?</h3>
