...
export default function AmazonPage() {
  ...
  async function fetchStatus() {
    ...
    try {
      const res = await fetch("/api/admin/amazon/status");
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Amazon Integration</h1>
        ...
        {statusLoading ? (
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700">SP-API Credentials: {status.configured ? "Configured" : "Not configured"}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${status.sellerConfigured ? "bg-green-500" : "bg-yellow-500"}`} />
              <span className="text-gray-700">Seller ID: {status.sellerConfigured ? "Configured" : "Not configured"}</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      {/* Sync controls */}
      <div className="bg-white rounded-lg border border-gray-200 p-5 mb-6">
        <h2 className="font-semibold text-gray-900 mb-4">Sync to Amazon</h2>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-4">
          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
            ...
            />
            Dry Run
            <span className="text-xs text-gray-400">(preview only, no Amazon changes)</span>
          </label>
        ...
        >
          {syncing ? "Syncing..." : dryRun ? "Preview Sync" : "Sync to Amazon"}
        </button>
      </div>
