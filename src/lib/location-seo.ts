...
export function generateSizePageIntro(
  ...
  const intros: Record<string, string> = {
    metro: `Looking for ${brandName} ${displaySize} tires near me in ${cityName}, ${stateAbbr}? Ship.Tires ships free to the ${cityName} metro area — one of the largest markets in ${stateName}. With ${climate}, choosing the right tires matters for the ${(population / 1000000).toFixed(1)}M+ residents navigating ${cityName} roads daily. ${vehicleText} Prices start at $${lowestPrice} per tire with free shipping — every ${brandName} tire is shipped directly to your door or local installer near you.`,
    large: `Find ${brandName} ${displaySize} tires near me in ${cityName}, ${stateAbbr} with free shipping on every order. As a major ${stateName} city, ${cityName} drivers face ${climate} — and the right tires make all the difference for safe daily driving. ${vehicleText} Starting at $${lowestPrice} per tire, we ship directly to your home or preferred ${cityName} tire shop. Free shipping, no exceptions.`,
    mid: `Get ${brandName} ${displaySize} tires shipped free to ${cityName}, ${stateAbbr}. Searching for tires near me? Every order ships free to ${cityName}. Drivers across the ${cityName} area deal with ${climate}, and ${brandName} tires are engineered to handle exactly these conditions. ${vehicleText} With prices from $${lowestPrice} per tire and free shipping on every order, getting quality tires shipped to ${cityName} has never been easier.`,
    small: `Need ${brandName} ${displaySize} tires near me in ${cityName}, ${stateAbbr}? Ship.Tires ships free even to smaller ${stateName} communities — no minimum order. With ${climate} to contend with, reliable tires are essential for ${cityName} drivers. ${vehicleText} Starting at just $${lowestPrice} per tire, we ship directly to your address or any installer near ${cityName}. Free shipping on every tire shipped.`,
  };
...
export function generateBrandPageIntro(
  ...
  const climate = getStateClimate(stateSlug);
  return `Looking for ${brand.name} tires near me in ${cityName}, ${stateAbbr}? Browse the full ${brand.name} tire lineup — all shipped free to ${cityName}. Founded in ${brand.founded} in ${brand.country}, ${brand.name} offers ${brand.models.length} tire lines across ${totalSizes} sizes — from all-season touring tires to high-performance and all-terrain options. ${cityName} drivers face ${climate}, and ${brand.name} engineers tires to deliver confident performance in every condition. Every ${brand.name} tire ships with free shipping to your door or any installer near ${cityName}. Ship tires today — delivered in 3-7 business days.`;
}
