...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-label-white shadow-sm border-b border-ink-grey/20">
          ...
          <div className="flex h-20 items-center lg:h-24">
            ...
            <div className="hidden lg:flex lg:items-center lg:gap-1 lg:ml-auto">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="rounded-md px-2.5 py-2 text-sm font-medium text-ink-grey transition-all hover:bg-rubber/5 hover:text-rubber whitespace-nowrap"
                >
                  {item.name}
                </Link>
              ))}
              <a
                ...
                className="inline-flex items-center gap-1.5 ml-3 text-sm font-mono font-semibold text-rubber hover:text-safety-orange transition-colors whitespace-nowrap"
              >
            ...
            <div className="flex items-center gap-3 ml-auto lg:hidden">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-md p-2 text-ink-grey hover:bg-rubber/5 hover:text-rubber"
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="border-t border-ink-grey/10 lg:hidden bg-label-white">
            <div className="space-y-1 px-4 pb-4 pt-2">
              {navigation.map((item) => (
                <Link
                  ...
                  className="block rounded-md px-3 py-2 text-base font-medium text-ink-grey hover:bg-rubber/5 hover:text-rubber transition-all"
                >
                  {item.name}
                </Link>
              ))}
